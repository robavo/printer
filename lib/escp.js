...
// 1D barcode. ESC i [params] B [data] terminator.
// type: '0' CODE39, '1' ITF, '5' EAN/UPC, '6' UPC-E, '9' CODABAR, 'a' CODE128
//       (default), 'b' GS1-128. (Was `t${0x0A}` = "t10", an ITF request with a
//       stray '0' — never printed; fixed 2026-09-07 for the SSCC label.)
// height: dots (2 raw bytes).  width: 0=xs, 1=small, 2=medium, 3=large.
// showText: human-readable characters under the bars.
// CODE128 / GS1-128 data ends with three backslashes, every other type with one.
escp.barcode = (data, { type = 'a', height = 100, width = 2, showText = true } = {}) => {
  const hL = height & 0xFF
  const hH = (height >> 8) & 0xFF
  const params = Buffer.concat([
    Buffer.from(`t${type}r${showText ? 1 : 0}h`, 'latin1'),
    Buffer.from([hL, hH]),
    Buffer.from(`w${width}`, 'latin1'),
  ])
  const header = Buffer.from([0x1B, 0x69])
  const bMarker = Buffer.from([0x42])
  const dataBuf = Buffer.from(String(data), 'latin1')
  const footer = (type === 'a' || type === 'b') ? Buffer.from([0x5C, 0x5C, 0x5C]) : Buffer.from([0x5C])
  return Buffer.concat([header, params, bMarker, dataBuf, footer])
}
